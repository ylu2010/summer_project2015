...
void sample_parameters(int *x, int dim_num, int point_num, int seed_init)
{
	  int duplication = 5;
	  int i;
	  int j;
	...
	//
	  ...
	  sprintf(ilhs_filename, "ILHS.txt");
	  if(!(  fp=fopen("ILHS0.txt","w")))
	  {
		  printf("I can not open the file ILHS.txt.");
	      exit(1);
	  }
	  fprintf(fp, "%d %d\n", dim_num, point_num);
	  for ( j = 0; j < point_num; j++ )
	  {
		  ...
		  for ( i = 0; i < dim_num; i++ )
	      {
	        fprintf(fp, "%d ", x[i+j*dim_num]);
	      }
	      fprintf(fp, "\n");
	  }
	  fclose(fp);

	    printf("Desired minimum distance = %g \n", opt);
	    printf("Average minimum distance= %g\n", average);
	    printf("Standard deviation:=%g\n", std);
	    printf("Covariance=%g\n", covc);

	    ihs_write ( dim_num, point_num, duplication, seed_init, seed,
	      x, ilhs_filename );
